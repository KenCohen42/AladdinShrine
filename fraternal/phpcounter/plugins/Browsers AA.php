<?php
	echo "<h2>Graphical Browsers AA</h2>";
	...
	if(GetNumberOfHits()){
		...
		$CountTheRest = 0;
		$FACount = count($FA);
		for($i=$FACount-1; $i>=0; $i--){ //we start at count-1 because the very first one - the last line in the log - is empty
		...
		imagefilledrectangle($image, 0, 0 , 600, 500, $white);
		...
		imagestring($image, 5, 20, 420, "Generated on " .date("d M y h:i:s A", mktime()), $black);
		imageline($image, 0, 60, 600, 60, $black);
		
		$CentreLine = ($ImageH / 2);
		$MaxRadius = $ImageH - $CentreLine - 100;
		$CentreX = 200;
		$CentreY = $CentreLine;		
		...
		for($i=$SliceStart; $i<=$SliceEnd; $i+=0.05){
			$x = $MaxRadius * (sin(deg2rad($i)));
			$y = $MaxRadius * (cos(deg2rad($i)));
			imageline($image, $CentreX, $CentreY, $x+$CentreX, $CentreLine-$y, $Col);
			}
			imagestring($image, 5, 370, $ListPos, $key . " (" .number_format((100 * $PC), 2, ".", "") . "%)", $Col);
			$SliceStart = $SliceEnd;
			
			$ListPos += 20;
			}
		imagepng($image, "./gbrowsers.png");
		echo "<p><img src=\"gbrowsers.png?" . time() . "\" alt=\"Graphical Browsers\"/></p>";
		...
		for($i=$SliceStart; $i<=$SliceEnd; $i+=0.01){
			$x = $MaxRadius * (sin(deg2rad($i)));
			$y = $MaxRadius * (cos(deg2rad($i)));
			imageline($image, $CentreX, $CentreY, $x+$CentreX, $CentreLine-$y, $Col);
			}
		
		imagestring($image, 5, 370, $ListPos, "Internet Explorer" . " (" .number_format((100 * $PCIE), 2, ".", "") . "%)", $Col);
		$SliceStart = $SliceEnd;
		...
		imagestring($image, 5, 370, $ListPos+20, "Gecko Browsers" . " (" .number_format((100 * $PCGecko), 2, ".", "") . "%)", $Col);
		$SliceStart = $SliceEnd;
		...
		imagestring($image, 5, 370, $ListPos+40, "Others" . " (" .number_format((100 * $PCOther), 2, ".", "") . "%)", $Col);
		imagepng($image, "./gbrowsers2.png");
		echo "<p><img src=\"gbrowsers2.png?" . time() . "\" alt=\"Graphical Browsers\"/></p>";
