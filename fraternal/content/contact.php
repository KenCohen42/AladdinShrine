<table border="1" width="100%">
  <tr> 
    <td colspan="4" align="center"><font size="4" COLOR="#990000"><strong>Phone 
      (614) 475-2609 Toll Free (800) 475-3850 Fax (614) 475-8225</strong></font></td>
  </tr>
  <tr> 
    <th>Staff Member</th>
    <th>Position</th>
    <th>Email</th>
    <th>Extension</th>
	<th>Direct Dial</th>
	<th>Other Contact Info</th>
  </tr>
  <tr>
    <td>Membership</td>
	<td colspan="5">Amber ext. 113</td>
  </tr>
  <tr> 
    <td>Chad Dennewitz</td>
    <td>Executive Director</td>
    <td><a href="mailto:cdennewitz@aladdinshrine.org">cdennewitz@aladdinshrine.org</a></td>
    <td>ext. 110</td>
  </tr>
  <tr> 
    <td>Brenda Springer</td>
    <td>Marketing Manager</td>
    <td><a href="mailto:bspringer@aladdinshrine.org">bspringer@aladdinshrine.org</a></td>
    <td>ext. 101</td>
  </tr>
  <tr> 
    <td>Ted Eisleben</td>
    <td>Fraternal Administrator</td>
    <td><a href="mailto:teisleben@aladdinshrine.org">teisleben@aladdinshrine.org</a></td>
    <td>ext. 102</td>
  </tr>
  <tr> 
    <td>Kim Luckeydoo</td>
    <td>Circus, ASHAC and Shrine Hospital Contact</td>
    <td><a href="mailto:kluckeydoo@aladdinshrine.org">kluckeydoo@aladdinshrine.org</a></td>
    <td>ext. 103</td>
	<td>Circus: (614) 269-0246</td>
  </tr>
  <tr> 
    <td>Amber Crosby</td>
    <td>Membership, Shrine Event Coordinator</td>
    <td><a href="mailto:acrosby@aladdinshrine.org">acrosby@aladdinshrine.org</a></td>
    <td>ext. 113</td>
	<td></td>
  </tr>
  <tr> 
    <td>Elizabeth Shoemaker</td>
    <td>Accountant</td>
    <td><a href="mailto:eshoemaker@aladdinshrine.org">eshoemaker@aladdinshrine.org</a></td>
    <td>ext. 112</td>
  </tr>
  <tr> 
    <td>Deb Rose</td>
    <td>Catering by Design</td>
    <td><a href="mailto:drose@AVIFoodsystems.com">drose@AVIFoodsystems.com</a></td>
    <td>ext 124 </td>
	<td>(614) 471-2260</td>
	<td>Cell: (330) 806-8991 Fax: (614) 471-2776</td>
  </tr>
  <tr> 
    <td>W. Garol Rogers</td>
    <td>Potentate</td>
    <td><a href="mailto:billamspaugh@gmail.com">grogers@aladdinshrine.org</a></td>
    <td>ext. 104</td>
  </tr>
  <tr> 
    <td>Don Goodman</td>
    <td>Recorder</td>
    <td><a href="mailto:dgoodman@columbus.rr.com">dgoodman@columbus.rr.com</a></td>
    <td>ext. 105</td>
  </tr>
  <tr> 
    <td>Dennis Feyh</td>
    <td>Treasurer</td>
    <td><a href="mailto:ddf6@hotmail.com">ddf6@hotmail.com</a></td>
    <td>ext. 109</td>
  </tr>
  <tr>
    <td>John Morris</td>
    <td>Circus Director</td>
    <td><a href="mailto:jmorris@aladdinshrine.org">jmorris@aladdinshrine.org</a></td>
    <td>ext. 117</td>
  </tr>
  <tr> 
    <td>Building Security</td>
    <td>After hours</td>
    <td>614-475-2609</td>
    <td>ext. 111</td>
  </tr>
</table>
